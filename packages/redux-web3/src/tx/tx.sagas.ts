...
import { TxBroadcasted, TxConfirmed, TxError, TxReceipt, TxSendAction, TxSendRawAction } from './tx.actions';
...
function* sendTransaction(action: TxSendAction): SagaIterator {
  ...
  return eventChannel(
    (emit: (arg?: any) => void): Unsubscribe => {
      ...
      try {
        _transactionEvents = action.web3.eth
          .sendTransaction(action.txArgs)
          .on(
            'transactionHash',
            (_transaction_hash: string): void => {
              ...
              Object.keys(action.txArgs).forEach(
                (key: string): void => {
            ...
            (_amount: number, _receipt: any): void => {
              ...
              if (state.backlink.status !== 'CONNECTED' && state.backlink.status !== 'LOADING') {
                if (!(_amount % 5) || _amount < 5) {
                  if (action.txArgs.from) emit(AccountUpdateRequest(action.txArgs.from));
                  if (action.txArgs.to) emit(AccountUpdateRequest(action.txArgs.to));
                }
              }
          ...
          .on(
            'receipt',
            (_receipt: any): void => {
              action.web3.eth.getTransaction(transaction_hash).then(
                (txInfos: any): void => {
          ...
          .on(
            ...
            (_error: any): void => {
              ...
              emit(
                FeedNewError(_error, _error.message, '[tx.sagas.ts][sendTransaction] Trying to send a transaction.'),
              );
              if (action.resolvers) {
        ...
        Vortex.get().Store.dispatch(
          FeedNewError(reason, reason.message, '[tx.sagas.ts][sendTransaction] Trying to send a transaction.'),
        );
        if (action.resolvers) {
      ...
      return (): void => {
        if (_transactionEvents) _transactionEvents.off();
      };
    },
  );
}
...
function* sendRawTransaction(action: TxSendRawAction): SagaIterator {
  ...
  return eventChannel(
    (emit: (arg?: any) => void): Unsubscribe => {
      ...
      try {
        _transactionEvents = action.web3.eth
          .sendRawTransaction(action.signedTx)
          .on(
            'transactionHash',
            (_transaction_hash: string): void => {
              transaction_hash = _transaction_hash;
            ...
            (_error: any): void => {
              ...
              emit(
                FeedNewError(
                  _error,
                  _error.message,
                  '[tx.sagas.ts][sendRawTransaction] Trying to send a raw transaction.',
                ),
              );
              if (action.resolvers) {
                action.resolvers.error(transaction_hash);
        ...
        Vortex.get().Store.dispatch(TxError(transaction_hash, reason));
        Vortex.get().Store.dispatch(
          FeedNewError(reason, reason.message, '[tx.sagas.ts][sendRawTransaction] Trying to send a raw transaction.'),
        );
        if (action.resolvers) {
          action.resolvers.error(transaction_hash);
          action.resolvers = undefined;
