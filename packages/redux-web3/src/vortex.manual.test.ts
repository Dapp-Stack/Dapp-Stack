...
describe('Vortex', () => {
    ...
    'Add Event',
      ...
      setTimeout((): void => {
        Vortex.get().Store.dispatch(
          EventAdd('Migrations', Migrations.networks[Object.keys(Migrations.networks)[0]].address, 'Test'),
        );
        done();
      }, 10000);
    },
  ...
  test(
    ...
    (done: any): void => {
      ...
      contract.vortexMethods.setCompleted.send(23, { from: coinbase }).then(
        (_txHash: string): void => {
          let intervalId = setInterval(() => {
            const state = Vortex.get().Store.getState();
            switch (state.feed.length) {
        ...
        switch (state.feed.length) {
          case 9:
            if (
              ...
              (<FeedNewContractState>state.feed[8]).contract_name === 'Migrations' &&
