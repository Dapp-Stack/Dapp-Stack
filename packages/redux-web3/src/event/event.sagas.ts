    ...
    (emit: (arg?: any) => void): Unsubscribe => {
      ...
      const subscription_creator = async (): Promise<void> => {
        try {
          ...
          subscription = link.eth.subscribe(
            'logs',
            {
...
function* onEventAdd(action: EventAddAction): SagaIterator {
  ...
  if (state.backlink.status !== 'CONNECTED') {
    const error =
      "[event.sagas.ts][onEventAdd] Request Event listening while backlink is not connected. Events won't be fetched.";
    console.warn(error);
    return;
  }
  if (
    !state.contracts ||
    !state.contracts[action.contract_name] ||
    !state.contracts[action.contract_name].artifact ||
    !state.contracts[action.contract_name].artifact.abi
  ) {
    const error = new Error('Request Event listening on unknown contract type ' + action.contract_name);
    console.error(error);
    yield put(
      FeedNewError(error, error.message, '[event.sagas.ts][onEventAdd] Trying to add event ' + action.event_name),
    );
  }
  let found;
  for (let abi_idx = 0; abi_idx < state.contracts[action.contract_name].artifact.abi.length; ++abi_idx) {
    if (
      state.contracts[action.contract_name].artifact.abi[abi_idx].type === 'event' &&
      state.contracts[action.contract_name].artifact.abi[abi_idx].name === action.event_name
    ) {
  ...
  const create_subscription = yield call(
    createSubscription,
