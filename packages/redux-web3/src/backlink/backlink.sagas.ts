...
function recursiveBackwardFetcher(
  web3: any,
  block: any,
  height: number,
  depth: number,
    ...
    web3.eth
      ...
      .then((_block: any) => {
        if (_block && _block.transactions) {
          ...
          emit(
            FeedNewError(new Error(msg), msg, '[backlink.sagas.ts][recursiveBackwardFetcher] Trying to fetch block'),
          );
          emit(END);
        }
      })
      .catch((_error: Error) => {
        if (depth && height >= 1) {
          ...
          emit(
            FeedNewError(_error, _error.message, '[backlink.sagas.ts][recursiveBackwardFetcher] Trying to fetch block'),
          );
          emit(END);
        }
      });
