...
const BacklinkConnectedReducer: Reducer<BacklinkState, BacklinkConnectedAction> = (
  state: BacklinkState,
...
const BacklinkErrorReducer: Reducer<BacklinkState, BacklinkErrorAction> = (
  state: BacklinkState,
...
const BacklinkDisconnectedReducer: Reducer<BacklinkState, BacklinkDisconnectedAction> = (
  state: BacklinkState,
  action: BacklinkErrorAction,
): BacklinkState => {
  return {
...
const BacklinkDisableReducer: Reducer<BacklinkState, BacklinkDisableAction> = (
  state: BacklinkState,
...
const BacklinkCreateHookReducer: Reducer<BacklinkState, BacklinkCreateHookAction> = (
  state: BacklinkState,
  action: BacklinkCreateHookAction,
): BacklinkState => {
  if (!state.hooks) state.hooks = {} as BacklinkHookState;
  ...
  return {
    ...
    hooks: {
      ...state.hooks,
      [action.address.toLowerCase()]: [...state.hooks[action.address.toLowerCase()]],
    },
  } as BacklinkState;
};

const BacklinkRemoveHookReducer: Reducer<BacklinkState, BacklinkRemoveHookAction> = (
  state: BacklinkState,
...
export const backlink: Reducer<BacklinkState, BacklinkActions> = (
  state: BacklinkState = { status: 'DISABLED' } as BacklinkState,
