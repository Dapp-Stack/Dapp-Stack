...
export const getContract = (state: State, contract_name: string, contract_address?: string, load?: boolean): any => {
  ...
  if (contract_address) {
    ...
    if (!state.contracts[contract_name]) throw new Error('Unknown contract type ' + contract_name);
    if (
      !state.contracts[contract_name][contract_address] ||
      !state.contracts[contract_name][contract_address].instance
    ) {
...
export const callContract = (instance: any, method_name: string, ...args: any[]): any => {
  ...
  if (!instance.vortexMethods[method_name]) throw new Error('Unknown method ' + method_name);
  if (!instance.vortexMethods[method_name].data) throw new Error('Method ' + method_name + ' is not constant');
...
export const getFeed = (state: State, ...filter: FeedType[]): FeedState[] => {
  ...
  if (!filter.length)
    filter_sum =
      FeedType.Transactions | FeedType.Contracts | FeedType.Accounts | FeedType.Errors | FeedType.IPFSContent;
  else {
...
export const getEvents = (
  ...
  let signature = '';
  if (!config) signature = 'none:none:none';
  else {
    signature += (config.event_name || 'none') + ':';
    signature += (config.contract_name || 'none') + ':';
    if (config.contract_address) config.contract_address = config.contract_address.toLowerCase();
  ...
  if (
    ...
    !eventFeedSubscriptions[signature]
  ) {
