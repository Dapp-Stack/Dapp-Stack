...
const getWeb3: Promise<any> = new Promise<any>(
...
describe('Vortex', () => {
    ...
    const vtx = Vortex.factory(
      {
        type: 'embark',
        embark_contracts: {
          SimpleStorage: new _web3.eth.Contract(
            SimpleStorage.abiDefinition,
            '0xc7E4d433eb912f78244A54bac86ba757E4e8641F',
            {
  ...
  test(
    'Send New Transaction from dispatch',
    (done: (arg?: any) => void) => {
  ...
  test(
    'Send New Transaction from web3',
    (done: (arg?: any) => void) => {
      _web3.eth.getAccounts().then((acc: string[]) => {
  ...
  test(
    'Call State modifying method, expect txHash and new tx',
    (done: any): void => {
      const state = Vortex.get().Store.getState();
      ...
      contract.vortexMethods.set.send(23, { from: coinbase }).then(
        (_txHash: string): void => {
          let intervalId = setInterval(() => {
            const state = Vortex.get().Store.getState();
            switch (state.feed.length) {
  ...
  test(
    'Load new instance of Migrations',
    (done: (arg?: any) => void) => {
      ...
      let intervalId = setInterval(() => {
        ...
        switch (state.feed.length) {
          case 9:
            if (
              state.feed[8].action === 'NEW_CONTRACT' &&
              (<FeedNewContractState>state.feed[8]).contract_name === 'SimpleStorage' &&
              (<FeedNewContractState>state.feed[8]).contract_address ===
                (<Web3LoadedState>Vortex.get().Store.getState().web3).coinbase
            )
              done();
            else done(new Error('Invalid Feed element'));
            break;
          default:
            break;
        }
  ...
  test(
    'Recover IPFS hash previously uploaded',
    done => {
