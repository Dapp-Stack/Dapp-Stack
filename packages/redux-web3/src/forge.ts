...
export function forge<T extends State = State>(
  contracts: EmbarkContracts | TruffleContracts | ManualContracts,
  config: GeneratorConfig<T> = undefined,
): Store {
  ...
  switch (contracts.type) {
    case 'truffle':
      ...
      for (let idx in truffle_contracts.truffle_contracts) {
        (<ContractArtifactState>(<any>initialState).contracts)[
          truffle_contracts.truffle_contracts[idx].contractName
        ] = {
          artifact: {
    ...
    case 'embark':
      ...
      for (let idx in Object.keys(embark_contracts.embark_contracts)) {
        (<ContractArtifactState>(<any>initialState).contracts)[Object.keys(embark_contracts.embark_contracts)[idx]] = {
          artifact: {
            abi:
              embark_contracts.embark_contracts[Object.keys(embark_contracts.embark_contracts)[idx]].options
                .jsonInterface,
            bytecode:
              embark_contracts.embark_contracts[Object.keys(embark_contracts.embark_contracts)[idx]].options.data,
      ...
      for (let idx in Object.keys(manual_contract.manual_contracts)) {
        (<ContractArtifactState>(<any>initialState).contracts)[Object.keys(manual_contract.manual_contracts)[idx]] = {
          artifact: {
            ...
            bytecode:
              manual_contract.manual_contracts[Object.keys(manual_contract.manual_contracts)[idx]].deployed_bytecode,
  ...
  const store: Store<T> = createStore<T, any, any, any>(
    combinedReducer,
    combinedInitialState,
    composer(applyMiddleware(sagaMiddleware)),
  );

  if (config && config.custom_sagas) sagaMiddleware.run(rootSagaBuilder(...config.custom_sagas), store.dispatch);
  else sagaMiddleware.run(rootSagaBuilder(), store.dispatch);
