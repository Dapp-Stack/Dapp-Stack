...
export const feed: Reducer<FeedState[], FeedActions> = (
  ...
  switch (action.type) {
    ...
    case 'FEED_NEW_ERROR':
      console.warn(
        '[Feed Error]: ' + (<FeedNewErrorAction>action).message + ' => ' + (<FeedNewErrorAction>action).when,
      );
      state.push({
        action: 'NEW_ERROR',
        error: {
