...
function* onLoadRequest(action: IPFSLoadAction): SagaIterator {
  ...
  if (!config.active) {
    yield put(
      FeedNewError(
        new Error('IPFS not active'),
        'IPFS not active',
        '[ipfs.saga.ts][IPFSFetchData] Trying to fetch ipfs hash ' + action.hash,
      ),
    );
    return;
  }
  if (!action.hash || !IsIPFS.multihash(action.hash)) {
    yield put(
      FeedNewError(
        new Error('Invalid IPFS Hash ' + action.hash),
        'Invalid IPFS Hash ' + action.hash,
        '[ipfs.saga.ts][onLoadRequest] Trying to fetch ipfs hash ' + action.hash,
      ),
    );
    return;
  }
  const exists = ipfs[action.hash];
  if (exists && exists.content) return;
