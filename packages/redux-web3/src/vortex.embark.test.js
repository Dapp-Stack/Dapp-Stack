...
var SimpleStorage = require("../../setup/embark/dist/contracts/SimpleStorage.json");
var Chains = require("../../setup/embark/chains.json");
...
describe("Vortex", function () {
    ...
    test('Recover Instance', function () {
        expect(vortex_1.Vortex.get().Contracts.embark_contracts["SimpleStorage"]).not.toBe(undefined);
    });
    ...
    test('Add Event', function (done) {
        setTimeout(function () {
            vortex_1.Vortex.get().Store.dispatch(event_actions_1.EventAdd("SimpleStorage", SimpleStorage.deployedAddress, "Test"));
            done();
        }, 10000);
    }, 30000);
    ...
    test('Call State modifying method, expect txHash and new tx', function (done) {
        ...
        var contract = state.contracts[contractName][contractAddress].instance;
        contract.vortexMethods.set.send(23, { from: coinbase }).then(function (_txHash) {
            var intervalId = setInterval(function () {
                var state = vortex_1.Vortex.get().Store.getState();
                switch (state.feed.length) {
                    case 8:
    ...
    test('Load new instance of Migrations', function (done) {
        vortex_1.Vortex.get().loadContract("SimpleStorage", vortex_1.Vortex.get().Store.getState().web3.coinbase);
        var intervalId = setInterval(function () {
            var state = vortex_1.Vortex.get().Store.getState();
            switch (state.feed.length) {
                case 9:
                    if (state.feed[8].action === 'NEW_CONTRACT' && state.feed[8].contract_name === 'SimpleStorage' && state.feed[8].contract_address === vortex_1.Vortex.get().Store.getState().web3.coinbase)
                        done();
                    else
...
//# sourceMappingURL=vortex.embark.test.js.map