...
import { TxBroadcasted, TxConfirmed, TxError, TxReceipt } from '../tx/tx.actions';
import { Vortex } from '../vortex';
import { AccountUpdateRequest } from '../accounts/accounts.actions';
import { BN } from 'bn.js';

const toLower: string[] = ['to', 'from', 'gas', 'gasPrice', 'value'];

export function runForceRefreshRoundOn(
  state: State,
  emit: (arg?: any) => void,
  contractName: string,
  instance_address: string,
): void {
  Object.keys(state.contracts[contractName][instance_address].instance.vortexMethods).forEach(
    (methodName: string): void => {
      if (state.contracts[contractName][instance_address].instance.vortexMethods[methodName].cache) {
        Object.keys(state.contracts[contractName][instance_address].instance.vortexMethods[methodName].cache).forEach(
          (signature: string): void => {
            if (
              state.contracts[contractName][instance_address].instance.vortexMethods[methodName].cache[signature]
                .synced &&
              !state.contracts[contractName][instance_address].instance.vortexMethods[methodName].cache[signature]
                .disable_refresh
            ) {
...
export function runForceRefreshRound(state: State, emit: (arg?: any) => void): void {
  Object.keys(state.contracts).forEach(
    (contractName: string): void => {
      Object.keys(state.contracts[contractName]).forEach(
        (instance_address: string): void => {
...
const compareBytecode = (address: string, bytecode: string, web3: any): Promise<boolean> => {
  return new Promise(
    (ok: (arg: any) => void, ko: (arg: any) => void): void => {
      web3.eth
        .getCode(address)
        .then(
          (remote_bytecode: string): void => {
            if (remote_bytecode.indexOf('0x') === 0) remote_bytecode = remote_bytecode.substr(2);
            if (bytecode.indexOf('0x') === 0) bytecode = bytecode.substr(2);
    ...
    case 'truffle':
      try {
        ...
        for (let idx = 0; idx < contractNames.length; ++idx) {
          ...
          for (
            let save_contract_idx = 0;
            save_contract_idx < contracts.config.config.contracts.length;
            ++save_contract_idx
          ) {
          ...
          if (
            contract_instance &&
            contract_instance.networks &&
            contracts.config.config.preloaded_contracts.indexOf(contractNames[idx]) !== -1
          ) {
            ...
            yield* loadContract(
              contractNames[idx],
              contract_instance.networks[action.networkId].address.toLowerCase(),
              action.coinbase,
              action._,
            );
            yield put(ContractSetDeployed(contractNames[idx], contract_instance.networks[action.networkId].address));
            recap.push({
              name: contractNames[idx],
              address: contract_instance.networks[action.networkId].address.toLowerCase(),
            });
          }
        }
...
function* contractCall(action: ContractCallAction, tx: any, arg_signature: string): SagaIterator {
  return eventChannel(
    (emit: (arg?: any) => void): Unsubscribe => {
      tx.call(action.transactionArgs)
        .then((result: any) => {
          emit(
            ContractVarReceived(action.contractName, action.contractAddress, action.methodName, arg_signature, result),
          );
          if (action.resolvers) action.resolvers.success(result);
        })
        .catch((error: any) => {
          emit(
            ContractVarErrorReceived(
          ...
          emit(
            FeedNewError(
              error,
              error.message,
              '[contracts.saga.ts][contractCall] Trying to recover constant call result.',
            ),
          );
          if (action.resolvers) action.resolvers.error(error);
        });
...
function* onContractCall(action: ContractCallAction): SagaIterator {
  ...
  if (!current_contract.vortexMethods[action.methodName].cache[arg_signature].synced) {
    const ctcall = yield call(
      contractCall,
      action,
      current_contract.methods[action.methodName](...action.methodArgs),
      arg_signature,
    );
    try {
      while (true) {
...
function* contractSend(action: ContractSendAction, tx: any, web3: any): SagaIterator {
  ...
  return eventChannel(
    (emit: (arg?: any) => void): Unsubscribe => {
      ...
      try {
        tx_events = tx
          .send(action.transactionArgs)
          .on(
            'transactionHash',
            (_transaction_hash: string): void => {
              ...
              Object.keys(action.transactionArgs).forEach(
                (key: string): void => {
          ...
          .on(
            'confirmation',
            (_amount: number, _receipt: any): void => {
              ...
              if (state.backlink.status !== 'CONNECTED' && state.backlink.status !== 'LOADING') {
                if (!(_amount % 5) || _amount < 5) {
                  ...
                  if (action.transactionArgs.from) emit(AccountUpdateRequest(action.transactionArgs.from));
                  if (action.transactionArgs.to) emit(AccountUpdateRequest(action.transactionArgs.to));
                }
              }
              if (_amount >= 24) emit(END);
            },
          )
          .on(
            'receipt',
            (_receipt: any): void => {
              web3.eth.getTransaction(transaction_hash).then(
                (txInfos: any): void => {
                  Vortex.get().Store.dispatch(
                    TxReceipt(transaction_hash, _receipt, {
            ...
            (_error: any): void => {
              ...
              emit(
                FeedNewError(
                  _error,
                  _error.message,
                  '[contracts.sagas.ts][contractSend.error] Trying to send method call.',
                ),
              );
              if (action.resolvers) {
        ...
        Vortex.get().Store.dispatch(
          FeedNewError(reason, reason.message, '[contracts.sagas.ts][contractSend.catch] Trying to send method call.'),
        );
        if (action.resolvers) {
          action.resolvers.error(transaction_hash);
          action.resolvers = undefined;
...
function* onContractSend(action: ContractSendAction): SagaIterator {
  ...
  const ctsend = yield call(
    contractSend,
    action,
    current_contract.methods[action.methodName](...action.methodArgs),
    current_state.web3._,
  );
  try {
    while (true) {
