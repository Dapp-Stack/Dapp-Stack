...
describe('Contracts Reducers', (): void => {
  ...
  test('ContractVarReceived', (): void => {
    state = contracts(
      state,
      ContractVarReceived(contractName, contractAddress, contractMethod, contractMethod, contractInstance),
    );
    expect(
      state[contractName][contractAddress].instance.vortexMethods[contractMethod].cache[contractMethod].data.test,
    ).toBe('TEST');
  });
  ...
  test('ContractVarErrorReceived', (): void => {
    state = contracts(
      state,
      ContractVarErrorReceived(contractName, contractAddress, contractMethod, contractMethod, contractInstance),
    );
    expect(
      state[contractName][contractAddress].instance.vortexMethods[contractMethod].cache[contractMethod].error.test,
    ).toBe('TEST');
  });
});
