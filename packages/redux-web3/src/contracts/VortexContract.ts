...
const TransactionArgumentList = ['from', 'to', 'gas', 'gasPrice', 'value', 'data', 'nonce'];
...
export class VortexContract {
  ...
  private getDataWithoutRefresh(methodName: string, ...methodArguments: any[]): any {
    ...
    if (methodArguments.length && typeof methodArguments[methodArguments.length - 1] === 'object') {
      for (
        let attribute_idx = 0;
        attribute_idx < Object.keys(methodArguments[methodArguments.length - 1]).length;
        ++attribute_idx
      ) {
        if (
          TransactionArgumentList.indexOf(Object.keys(methodArguments[methodArguments.length - 1])[attribute_idx]) ===
          -1
        ) {
          break;
        }
    ...
    _this.vortexMethods = Vortex.get().Store.getState().contracts[_this.artifact.name][
      _this._address.toLowerCase()
    ].instance.vortexMethods;
    ...
    if (_this.vortexMethods[methodName]) {
      if (_this.vortexMethods[methodName].cache[signature]) {
        ...
        if (!_this.vortexMethods[methodName].cache[signature].synced && !this._waiting_calls[methodName + signature]) {
          ...
          _this.vortexMethods[methodName].cache[signature].synced &&
          this._waiting_calls[methodName + signature]
        )
          this._waiting_calls[methodName + signature] = false;
      } else {
  ...
  private vortexCall(methodName: string, methodAbiIndex: number, ...methodArguments: any[]): Promise<any> {
    ...
    const _ret: Promise<any> = new Promise<any>(
    ...
    for (let abi_idx = 0; abi_idx < artifact.abi.length; ++abi_idx) {
      if (artifact.abi[abi_idx].type === 'function') {
        ...
        if (artifact.abi[abi_idx].constant) {
          _this.vortexMethods[artifact.abi[abi_idx].name].call = this.vortexCall.bind(
            this,
            artifact.abi[abi_idx].name,
            abi_idx,
          );
          ...
          _this.vortexMethods[artifact.abi[abi_idx].name].constantData = this.getDataWithoutRefresh.bind(
            this,
            artifact.abi[abi_idx].name,
          );
        } else {
          _this.vortexMethods[artifact.abi[abi_idx].name].send = this.vortexCall.bind(
            this,
            artifact.abi[abi_idx].name,
            abi_idx,
          );
