...
function fetchAccount(address: string, coinbase: boolean, emit: (arg?: any) => void): Promise<void> {
  return new Promise<void>(
    (ok: (arg?: any) => void, ko: (arg?: any) => void): void => {
      (<Web3LoadedState>Vortex.get().Store.getState().web3)._.eth
        .getBalance(address)
        .then((balance: any) => {
...
function loopOnAccounts(emit: (arg?: any) => void): Promise<void> {
  return new Promise<void>(
    (ok: (arg?: any) => void, ko: (arg?: any) => void): void => {
      const refresh_rate: number = (<AccountConfigState>Vortex.get().Store.getState().accounts.configuration)
        .refresh_rate;
      const accounts: AccountStoreState = Vortex.get().Store.getState().accounts;
      setTimeout((): void => {
        Object.keys(accounts)
          .filter((elem: string) => elem !== 'coinbase' && elem !== 'configuration')
          .forEach(
            (address: string): void => {
                  ...
                  (e: Error): void => {
                    ...
                    emit(
                      FeedNewError(
                        e,
                        e.message,
                        '[accounts.sagas.ts][loopOnAccounts] Trying to fetch account informations.',
                      ),
                    );
                    ko(e);
                  },
        ...
        if (running)
          loopOnAccounts(emit).catch(
            (e: Error): void => {
              emit(
                FeedNewError(e, e.message, '[accounts.sagas.ts][loopOnAccounts] Trying to fetch account informations.'),
              );
              ko(e);
            },
          );
...
function* singleFetch(action: AccountAddAction, new_address: boolean, coinbase: boolean): SagaIterator {
  return eventChannel((emit: (arg?: any) => void) => {
    fetchAccount(action.address.toLowerCase(), coinbase, emit)
      .then(
        (): void => {
          if (new_address) {
