...
const ls: Epic<IpfsAction, IpfsAction, IpfsState> = (action$, state$) => action$.pipe(
...
const watchLs: Epic<IpfsAction, IpfsAction, IpfsState> = (action$) => action$.pipe(
  filter(isActionOf(files.ls)),
  map(files.request.ls.request)
);

const cat: Epic<IpfsAction, IpfsAction, IpfsState> = (action$) => action$.pipe(
...
const watchCat: Epic<IpfsAction, IpfsAction, IpfsState> = (action$) => action$.pipe(
  filter(isActionOf(files.cat)),
  map((action) => files.request.cat.request(action.payload))
);

const rm: Epic<IpfsAction, IpfsAction, IpfsState> = (action$) => action$.pipe(
  filter(isActionOf(files.request.cat.request)),
  switchMap(action =>
...
const watchRm: Epic<IpfsAction, IpfsAction, IpfsState> = (action$) => action$.pipe(
  filter(isActionOf(files.rm)),
  map((action) => files.request.rm.request(action.payload))
);

const touch: Epic<IpfsAction, IpfsAction, IpfsState> = (action$, state$) => action$.pipe(
...
const watchTouch: Epic<IpfsAction, IpfsAction, IpfsState> = (action$) => action$.pipe(
  filter(isActionOf(files.touch)),
  map((action) => files.request.touch.request(action.payload))
);

const mkdir: Epic<IpfsAction, IpfsAction, IpfsState> = (action$) => action$.pipe(
  filter(isActionOf(files.request.cat.request)),
  switchMap(action =>
...
const watchMkdir: Epic<IpfsAction, IpfsAction, IpfsState> = (action$) => action$.pipe(
  filter(isActionOf(files.mkdir)),
  map((action) => files.request.mkdir.request(action.payload))
);

const rmdir: Epic<IpfsAction, IpfsAction, IpfsState> = (action$) => action$.pipe(
  filter(isActionOf(files.request.cat.request)),
  switchMap(action =>
    from(api.rm(action.payload)).pipe(
...
const watchRmdir: Epic<IpfsAction, IpfsAction, IpfsState> = (action$) => action$.pipe(
